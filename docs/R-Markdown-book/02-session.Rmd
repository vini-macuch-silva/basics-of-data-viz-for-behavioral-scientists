...
This didn't work the way we intended. The problem is that the proportions are being calculated relative to the points on the scale, whereas what we want instead is for them to be calculated relative to each expression type. Let's try again but now let's add the aesthetic `group` to `geom_bar`. What this will do is that it will create a grouping variable based on the variable *type*, which should indicate that the proportion of responses is to be plotted relative to the expression type.


```{r, out.width="70%", fig.align='center'}
pre1 %>%
  ggplot(aes(rat)) +
   geom_bar(aes(y = ..prop.., fill = typ, group = typ), stat = "count", position = position_dodge())
```


In this plot, one can associate a visual pattern to a quantity which is immediately meaningful. Just by glancing at the plot one can quite easily understand that people rated metaphoric expressions as *not at all literal* in about 80% of the cases.
...
pre1 %>%
  ggplot(aes(rat)) +
  geom_bar(aes(y = ..prop.., group = typ), stat = "count") +
  facet_wrap( ~ typ)
```
...
Let's now turn our attention to the data from pre-study 2, which compared metonymic and literal expressions. Let's then plot the ratings using our latest `ggplot()` code.
...
Now that we've created a single data set with observations from both the pre-study on metaphors and and the pre-study on metonomies, let's plot the combined results. Let's go back to `facet_wrap()` and try to change our code from earlier to produce a plot with four facets.
...
We were able to achieve what we wanted by instructing `ggplot2` to facet the data by both the original data set, *study*, and the expression type, *type*. Notice how the order of the arguments in the `facet_wrap()` call impacted the resulting plot: *study* was plotted along the x-axis while *type* was plotted along the y-axis. The graph is already good as it is, but let's redesign the facet labels to aid visualization. For that, we create two vectors with the new names we want to assign to the labels. Then, we use `labeller()` to apply those new labels to the already existing plot.
...
In order to conclude our introduction to `ggplot2`, let's attempt to visualize the combined results with our split bar technique from earlier, exploring some new visualization possibilities.

```{r, out.width="70%", fig.align='center'}
norms %>%
  ggplot(aes(rat)) +
  geom_bar(aes(y = ..prop.., fill = typ, group = typ), position = position_dodge()) +
